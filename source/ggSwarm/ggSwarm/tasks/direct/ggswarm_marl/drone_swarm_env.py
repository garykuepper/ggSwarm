...
class GgswarmMarlEnv(DirectMARLEnv):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._prop_forces = torch.zeros(
            self.num_envs * self.cfg.num_agents,
            4,
            3,
            device=self.device
        )
        self._prop_torques = torch.zeros(
            self.num_envs * self.cfg.num_agents,
            4,
            3,
            device=self.device
    ...
    def _setup_scene(self):
        print("[DEBUG] GgswarmMarlEnv._setup_scene start")
        sys.stdout.flush()
        ...
        print("[DEBUG] Cloning environments...")
        logger.info("[DEBUG] Cloning environments...")
        sys.stdout.flush()
        self.scene.clone_environments(copy_from_source=True)

        # Phase 3: Initialize Articulation view
        prim_path = self.cfg.robot_cfg.prim_path
        print(f"[DEBUG] Initializing Articulation with path: {prim_path}")
        logger.info(f"[DEBUG] Initializing Articulation with path: {prim_path}")
        sys.stdout.flush()
        self.robot = Articulation(self.cfg.robot_cfg)
        print("[DEBUG] Articulation initialized")
        logger.info("[DEBUG] Articulation initialized")
        sys.stdout.flush()

        # Add articulation to scene
        self.scene.articulations["robot"] = self.robot

        print("[DEBUG] GgswarmMarlEnv._setup_scene done")
        logger.info("[DEBUG] GgswarmMarlEnv._setup_scene done")
        sys.stdout.flush()
    ...
    def _get_observations(self) -> dict[str, torch.Tensor]:
        # Get root states [num_envs, num_agents, ...]
        pos_w = self.robot.data.root_pos_w.view(
            self.num_envs, self.cfg.num_agents, 3
        )
        quat_w = self.robot.data.root_quat_w.view(
            self.num_envs, self.cfg.num_agents, 4
        )
        ...
        rel_pos_b, _ = subtract_frame_transforms(
            pos_w.view(-1, 3), quat_w.view(-1, 4), target_pos
        )
    ...
    def _get_rewards(self) -> dict[str, torch.Tensor]:
        ...
        for i, agent in enumerate(self.cfg.possible_agents):
            ...
            rew_pos = (
                torch.exp(-dist_to_goal[:, i] / 0.5) *
                self.cfg.rew_scale_pos
            )
            # Velocity penalties
            rew_vel = (
                torch.norm(lin_vel_b[:, i], dim=-1) * self.cfg.rew_scale_vel
            )
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None):
        ...
        random_pos[:, :, 2] = sample_uniform(
            0.5, 1.5, (num_resets, self.cfg.num_agents), self.device
        )
        ...
        root_state = self.robot.data.default_root_state[0].clone().repeat(
            num_resets,
            self.cfg.num_agents,
            1
        )
        ...
        self.robot.write_root_pose_to_sim(
            root_state[:, :, :7].view(-1, 7), robot_indices
        )
